...
namespace ShaCoin
{
	...
	{
		int index;
		/*time_t类型数据用来保存从1970年1月1日0时0分0秒到现在时刻的秒数*/
		time_t timestamp;
		std::list<Transactions> lst_ts;
		//上一个区块的proof+一个随机数的结果（其实也可以只用随机数表示）挖矿是用一个固定字符串+proof转换的字符串进行哈希的结果，如果有0就成功
		long int proof;
	...
	public:
		...
		std::string GetJsonFromBlock(Block &block);//json文本传输数据小，便于调试扩展
